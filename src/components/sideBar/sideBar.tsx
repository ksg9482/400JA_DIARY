...
const SideBar = (props: any) => {
    ...
    const searchBoxChange =
        (key: string) => (e: React.MouseEvent<HTMLSpanElement>) => {
            if (key === constants.keyWord) {
                ...
                document.getElementById('searchbox-keyword')?.classList.toggle('border-slate-800')
                document.getElementById('searchbox-date')?.classList.toggle('border-slate-800')
    ...
    */
    ...
    const sideBarHandler = () => {
        ...
        document.getElementById('sidebar-hide-button')?.classList.toggle('border-l-cyan-700')
    }
    return (
        <div className="flex w-full">
            ...
            <button id="sidebar-hide-button" className="fixed top-96 z-auto inline-block w-8 h-24 border-t-2 border-b-2 border-r-2 border-t-cyan-700 border-b-cyan-700 border-r-cyan-700 rounded-r-md outline-none bg-slate-400" onClick={sideBarHandler}> 
