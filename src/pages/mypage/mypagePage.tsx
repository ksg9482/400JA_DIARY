...
const Mypage = () => {
    ...
    const getUserData = async () => {
        const userData: any = await axios.get(`http://localhost:8080/api/user/me`, { withCredentials: true });
    }
    ...
    useEffect(() => {
        const mypageInit = async () => {
            ...
            const MypageForm = userData ? userData.data : [{
                email: '',
                diaryCount: ''
            }]
    ...
    return (
        <div className="border h-screen flex items-center justify-center flex-col lg:mt-0">
            <div>
                {userData.email}님의 마이페이지 입니다.
            </div>
            <div>
                오늘까지 총 
                <span>{userData.diaryCount}</span>
                개 일기를 쓰셨습니다.
            </div>
