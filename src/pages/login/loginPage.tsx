...
const Login = () => {
  ...
  const socialLoginHandler =
    ...
    async (e: React.MouseEvent<HTMLSpanElement /*이거 바뀜 */>) => {
      ...
      const kakaoOAuth = () => {
        const kakaoOAuthURL = `https://kauth.kakao.com/oauth/authorize?client_id=${config.KAKAO_REST_API_KEY}&redirect_uri=${config.KAKAO_REDIRECT_URI}&response_type=code`;
        window.location.href = kakaoOAuthURL;
      };

      const googleOAuth = () => {
        const googleOAuthURL =
          process.env.GOOGLE || "http://localhost:8080/google";
